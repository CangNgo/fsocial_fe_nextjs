...
import { getConversations, getMessages } from "@/lib/api/messages/messageApi";
...
export function MessageFeature() {
  ...
  useEffect(() => {
    ...
    setNewMessage(null);
  }, [user?.userId, handleGetAllConversation, setNewMessage]);

  // update conversations list when a new message arrives
  const updateConversations = (baseConversation: Partial<Conversation>) => {
  ...
  useEffect(() => {
    ...
    setNewMessage(null);
  }, [newMessage, pathname, updateConversations, setNewMessage, conversation, conversations]);
